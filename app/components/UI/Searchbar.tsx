...
import { ChangeEvent, MutableRefObject, Suspense, useEffect, useRef, useState } from "react";
      ...
      {loading ? (
        ...
        data.length > 0 && (
          <div className="text-[14px] mt-2">
            ...
            <div className="flex flex-col gap-y-1 border-b max-sm:border-none p-3 max-sm:p-2 border-whitish_border">
              {data.map(({ id, title, album, image, artist, preview }, index) => (
                <div
                  ...
                  <div className={`flex flex-col justify-between`}>
                    ...
                    <div className="flex flex-row items-center gap-x-[6px] text-[12px] w-fit text-accent font-medium">
                      {preview ? (
                        <button onClick={e => playPreviewUrl(preview, id, e)}>{playingTrack === id ? "Pause" : "Écouter un extrait"}</button>
                      ) : (
                        <a
            ...
            <div className="w-full flex-row flex gap-x-6 text-[12px] justify-between h-[44px] items-center max-sm:hidden">
              <div className="flex flex-row h-full w-full">
                <div className="flex flex-row items-center  border-whitish_border justify-center px-3 gap-x-2.5 w-1/3">
                ...
                <div className="flex flex-row items-center border-l border-whitish_border justify-center px-3 gap-x-2.5 w-1/3">
                  <button className="flex flex-row items-center justify-center">
                    <Select classes="w-[15px] h-[15px] fill-gray_border stroke-none rotate-[180deg]" />
                  </button>
