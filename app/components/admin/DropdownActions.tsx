...
import {AdminModalsBan, AdminModalsChangeRole, AdminModalsDelete} from "./AdminModals";
...
export default function DropdownActions({options, row}: DropdownActionsProps) {
    ...
    const [changeRoleHref, setChangeRoleHref] = useState<string>("");
    ...
    const handleOptionClick = (option: OptionType) => {
        ...
        if (option.required === "ban") {
            ...
            setBanHref(option.href);
        } else if (option.required === "changeRole") {
            setIsChangeRoleModalOpen(true);
            setChangeRoleHref(option.href);
        } else {
    ...
    return (
        <div className="inline-block text-left">
            ...
            {isOpen && (
                <div className="z-50 fixed min-w-[8rem] overflow-hidden rounded-md border bg-white p-1 shadow-md"
                     ref={ref}>
                    <div className="py-1">
            ...
            {isDeleteModalOpen &&
                <AdminModalsDelete row={row} href={banHref} onClose={() => setIsDeleteModalOpen(false)}/>}
            {isChangeRoleModalOpen && <AdminModalsChangeRole row={row} href={changeRoleHref}
