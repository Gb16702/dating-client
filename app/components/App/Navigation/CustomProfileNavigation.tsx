...
import {profileSectionStore, useSettingsStore} from "@/app/stores/profileSectionStore";
...
export function SettingsNavigation() {
    const {active, setActive} = useSettingsStore();
            ...
            <section className="flex flex-row w-full bg-white border-b border-whitish_border">
                <>
                    {array.map((item: array, i: number) => (
                        <button
                            key={i}
                            className={`${isActive(item.name.toLowerCase()) &&
                            "border-b border-accent_blue"
                            } h-[72px] px-8 flex items-center bg-white`}
                            onClick={() => setActive(item.name.toLowerCase())}
                        >
                            <div>
                                <h2
                                    className={`text-[14px] ${isActive(item.name.toLowerCase())
                                        ? "text-accent_blue"
                                        : "text-subtitle_foreground"
                                    }`}
                                >
