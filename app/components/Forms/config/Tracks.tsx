...
import Searchbar, {Data, stopPlayingTrack} from "../../UI/Searchbar";
        ...
        <>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <div className="flex flex-col w-[450px] max-sm:w-full pt-[20px] gap-y-3">
                    {selectedTracks.map((track, index) => (
                        <div key={track.id} className="flex flex-row gap-x-2 py-2 rounded-[12px]">
                            ...
                            <Image src={track.image} alt={track.title} width={70} height={70}
                ...
                <div className="flex gap-x-2 items-center mt-4">
                    <Button
                        variant={"default"}
                        customClasses={`mt-4 rounded-[9px] font-bold px-[25px] max-md:w-[100%] ${lastStepLoading ? "cursor-not-allowed opacity-50" : ""}`}
                        disabled={lastStepLoading}>
                        {lastStepLoading ? <Loader/> : "Terminer la configuration"}
                    </Button>
                </div>
            </form>
        </>
