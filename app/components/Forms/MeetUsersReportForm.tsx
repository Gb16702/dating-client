...
export default function MeetUsersReportForm({ uid }: MeetUsersReportFormProps): JSX.Element {
  ...
  const onSubmit: SubmitHandler<MeetUsersReportForm> = async ({ reported_user_id, reason, details }: MeetUsersReportForm) => {
    try {
      ...
      toast.custom(t => <Toast message="Erreur lors du signalement" type={response.ok ? "Succès" : "Erreur"} t={t} />);
    } catch (e) {
        ...
        <div className="flex flex-col gap-y-3">
          <div>
            ...
            <InputGroup name="Raison" focusColor="focus-within:outline-accent_blue">
              <Input type="text" additionalClasses="w-full" ariaLabel="Raison du signalement" id="reason" {...register("reason")} />
            </InputGroup>
          </div>
          <div>
          ...
          <div className="flex flex-row w-full max-sm:flex-col max-sm:gap-y-2">
            <Button variant="custom" customClasses="rounded-[8px] font-semibold px-6 bg-accent_blue  text-white w-1/4 bg-accent_blue max-sm:w-full">
              Signaler
            </Button>
            <Button type="button" variant="custom" customClasses="rounded-[8px] font-semibold px-6 text-[12px] text-accent_blue w-1/4 max-sm:w-full">
              Annuler
            </Button>
          </div>
        </div>
